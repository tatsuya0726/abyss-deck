...
- 全画面右上に常駐していた「⚙ 設定」ボタンを削除
- 音楽、コントローラー、フルスクリーン設定をタイトル画面の既存「設定」内へ統合
- タイトル画面では上部HUDを隠し、背景・ロゴ・開始メニューを中心にした簡潔な構成へ変更
- TVでは中央配置、スマホ横画面では左右2カラムへ自動切替
- スマホ横画面は縦スクロールなしですべての開始操作が収まるよう専用調整
- TV専用スコープ（`html.tv-mode`）のみ変更し、通常スマホ版には影響なし
- 現在のキャッシュ番号: `tv-input.js?v=16`、`tv-landscape.css?v=13`
