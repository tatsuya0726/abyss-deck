...
- `tv.html`をTV・スマホ共用URLへ変更
- 縦画面では通常スマホ版レイアウトを使用
- 横画面ではTV・スマホ横向きレイアウトを使用
- 端末を回転すると再読み込みなしで即時切替
- 縦画面ではTV専用のフォーカス枠、ゲームパッド移動、TV専用設定3項目を非表示
- 横画面へ戻すとコントローラー・音楽・フルスクリーン設定を再表示
- 縦画面で初めて開いた場合は通常スマホ版のホーム画面追加案内を維持
- 現在のキャッシュ番号: `tv-input.js?v=18`、`tv-landscape.css?v=14`
