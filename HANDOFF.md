...
### 横画面の贈り物・ふりがな・行動予測を調整

- 今後の変更対象を、明示がない限りPC・スマホとも横画面だけに限定
- 白い吹き出しと黄色の決定ボタン上では、ふりがなを黒字に統一
...
- 敵の表示サイズと通常・エリート・ボス区分に応じ、行動予測と敵画像の安全距離を自動調整
- 敵名を画像から分離した専用帯へ移し、大型・縦長・横長の敵でも重ならない配置に変更
- 現在のキャッシュ番号: `responsive-landscape.css?v=51`、`responsive-desktop.css?v=5`、`responsive-shell.js?v=42`
- 横画面のふりがなはレイアウトを変えず黒字へ統一し、暗色面では淡い縁取りで可読性を維持。結果のゴールド表記を拡大し、敵名の専用位置は保ったまま枠・背景を除去
- ふりがな調整前の正常なCSS内容を維持したまま、壊れた版を端末キャッシュから再利用しないよう未使用のキャッシュ番号へ更新

### タップと閉じるが同時発火する問題を修正

- 画面下端15%のタップを一律で閉じる操作へ変換していた安全処理が、下側のカード・ボタン操作と競合していたため完全撤廃
- 自動測定後は透明キャッチャーを750ms残し、iPhoneの互換クリックを吸収して下の操作へ抜けないよう修正
- 全画面の操作先は保存済み補正座標による単一判定へ統一
- 現在のキャッシュ番号: `responsive-shell.js?v=22`

### 全画面共通の実測式タッチ自動補正

- 横画面の全ボタンで見た目とDOM矩形がずれる実機報告に対応し、補正画面を開くと自動測定を開始
- 「ここをタップ」の見た目上の中央を1回押すと、その指座標とSafariが返すボタン矩形の中心差から左右・上下補正を自動算出して保存
- 閉じる・設定・タイトル・マップ・戦闘・カード・図鑑を含む全操作で補正後座標を最優先し、一部操作だけ生座標へ戻していた例外を撤廃
- データリセットのみ誤操作防止のため補正転送対象外を維持
- 現在のキャッシュ番号: `responsive-landscape.css?v=23`、`responsive-shell.js?v=21`、`enhance.js?v=262`

### Safariの壊れたボタン矩形を使わない閉じる操作

- 補正0で指位置マーカーは正しいのに「判定：ボタン以外」となる実機画像から、SafariがモーダルボタンのDOM矩形を誤っていることを確認
- 図鑑・設定・補正などのモーダルシェルを中央寄せグリッドから外し、画面四辺へ直接固定して描画位置とDOM矩形を再構築
- 一時追加した画面下端15%の閉じる安全経路は、下側コンテンツと競合したため後続修正で撤廃
- 現在のキャッシュ番号: `responsive-landscape.css?v=22`、`responsive-shell.js?v=20`

### Safari横画面の表示面とタップ判定面の分離を修正

- タップ位置の黄色マーカーは指に合う一方、見た目より上の「下へ」が判定された実機画像から、Touch座標ではなくパネルの描画面と判定面の分離と特定
- 横画面モーダルでは `backdrop-filter` と `-webkit-overflow-scrolling: touch` を無効化し、ボタンをSafariの別合成レイヤーからルートレイヤーへ戻した
- 見た目のぼかしと慣性スクロールよりタップ位置の一致を優先。通常の縦スクロールは維持
- 現在のキャッシュ番号: `responsive-landscape.css?v=21`

### タップ位置の可視化とSafariの判定レイヤー修正

- タッチ補正画面で、指の位置を黄色、補正後の判定位置を水色の光で表示
- 実際に反応対象となったボタンも水色に発光し、判定名を画面内に表示
- Safariの表示面と判定面のどちらがずれているか確認できる診断表示を追加
...
- 画面回転、ページ復帰、アプリの表示復帰、補正画面を開くタイミングで保存値を再読込
- ページを閉じる際とバックグラウンド移行時にも現在値を再保存
- 「中央に戻す」は2回押した場合だけ実行し、補正中の座標ずれによる誤リセットを防止
- 現在のキャッシュ番号: `responsive-shell.js?v=18`

### 補正画面の「設定へ戻る」が押せない問題を修正

- 上下補正が `+80px` などの状態で「設定へ戻る」を押すと、約80px上の「下へ」が反応する問題を修正
- 補正画面内だけ保存済み補正を無視していた条件を削除し、矢印・中央に戻す・反応確認・設定へ戻るにも現在の補正値を正しく適用
- 補正値が不適切な場合に備え、見た目上の閉じる位置が直接取得できた場合の退避動作は維持
- 現在のキャッシュ番号: `responsive-shell.js?v=17`
...
- 横画面では `safe-area-inset-left/right` を使用。インストール版iOSが回転後に両方0を返す場合も、短いタッチ画面では左右へ48〜58pxのフォールバック安全領域を確保し、どちら向きでもDynamic Islandと重ならないようにした
- 横画面のカード選択イベントは、マーク・タイトル・説明・選択数をコンパクト化し、残りの縦幅をカード一覧へ割り当てた。強化・削除・刻印の選択画面へ共通適用
- 手動／自動タッチ補正、全画面キャッチャー、タップの合成クリック再送をすべて撤去。1回のタップから別ボタンが同時発火する経路をなくした
- 保存済みの旧補正値は参照しないため、端末側に値が残っていても動作へ影響しない
- 現在のキャッシュ番号: `responsive-landscape.css?v=27`、`responsive-shell.js?v=23`、`enhance.js?v=263`
...
- 2026-09-26以降の変更は、ユーザーから明示がない限りPC・スマホとも横画面だけを対象にする
