...
window.abyssEnemyKind=e=>/クトゥル|ノクス/.test(e?.n)?'abyss':/イカ|クラーケン/.test(e?.n)?'tentacle':/ヒュドラ|リヴァイアサン/.test(e?.n)?'water':/グソクムシ|カニ/.test(e?.n)?'crush':'bite';
...
function sound(kind,hit,total,boss){const a=window.abyssFeedbackAudio?.();if(!a)return;a.resume?.().catch(()=>{});const p=profiles[kind]||profiles.slash,t=a.currentTime,fin=total>1&&hit===total,volume=(boss?.11:.085)*(fin?1.2:1)*(window.abyssSfxVolume?.()??1),shift=1+Math.min(hit-1,6)*.07;
