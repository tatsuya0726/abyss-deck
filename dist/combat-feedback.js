...
function sound(kind,hit,total,boss){const a=window.abyssFeedbackAudio?.();if(!a)return;a.resume?.().catch(()=>{});window.duckAbyssBgm?.(boss?620:total>1?420:340,boss?.18:.26);const p=profiles[kind]||profiles.slash,t=a.currentTime,fin=total>1&&hit===total,volume=(boss?.18:.14)*(fin?1.2:1)*(window.abyssSfxVolume?.()??1),shift=1+Math.min(hit-1,6)*.07;
