...
window.resetAbyssTactics=g=>{g.primedAttack=0;g.futureEnergy=0;g.bankBlock=0;g.doubleFirstCard=0;g.doubleFirstPending=0;g.doubleFirstStartsTurn=0;g.firstCardEchoReady=0;g.extraTurnDraw=0;g.turnPoisonGain=0;g.curseInversion=0;g.tacticalDiscardUsed=false;g.tacticalExhaustUsed=false;g.relicDiscardUsed=false;g.relicExhaustUsed=false;g.relicRecoveryUsed=false;g.pressurePearlUsed=false;g.tidalEnergyCarry=0;g.relicCostRestores=[];g.overdriveDrain=0;g.overdriveEnergy=0;g.overdriveDraw=0;if(has(g,'巨鯨の心臓'))g.hp=Math.max(1,g.hp-1);if(has(g,'呪海の炉'))g.hand.push('abysscurse');if(has(g,'喰らう海溝'))g.hand.push('abysscurse');if(has(g,'サイドパック'))window.drawAbyssCards?.(2);if(has(g,'呪紋の外殻'))g.thorns=(g.thorns||0)+3;if(has(g,'供物の真珠')&&g.enemy)g.enemy.vulnerable=(g.enemy.vulnerable||0)+1;if(has(g,'漂流者の糸')&&g.enemy)g.enemy.weak=(g.enemy.weak||0)+1;if(has(g,'巨獣の顎'))g.str=(g.str||0)+1;if(has(g,'オウムガイの護殻'))g.guard=(g.guard||0)+1};
window.beginAbyssTactics=g=>{g.primedAttack=0;if((g.doubleFirstPending||0)&&(g.turn||1)>=(g.doubleFirstStartsTurn||1)){g.doubleFirstCard=(g.doubleFirstCard||0)+g.doubleFirstPending;g.doubleFirstPending=0;g.doubleFirstStartsTurn=0}g.firstCardEchoReady=g.doubleFirstCard||0;g.tacticalDiscardUsed=false;g.tacticalExhaustUsed=false;g.relicDiscardUsed=false;g.relicExhaustUsed=false;g.relicRecoveryUsed=false;g.pressurePearlUsed=false;if(has(g,'永久潮汐機関')){if(g.turn===1&&g.hand.length)g.draw.push(g.hand.pop());else g.drawPenalty=(g.drawPenalty||0)+1}g.energy+=(g.futureEnergy||0)+(has(g,'呪海の炉')?1:0)+(has(g,'四皇の王冠')?1:0)+(g.tidalEnergyCarry||0)+(g.silentAnchorEnergy||0);g.futureEnergy=0;g.tidalEnergyCarry=0;g.silentAnchorEnergy=0;if(g.turnPoisonGain&&g.enemy){g.poison=(g.poison||0)+g.turnPoisonGain;if(g.runStats)g.runStats.poisonApplied=(g.runStats.poisonApplied||0)+g.turnPoisonGain;let log=document.getElementById('battlelog');if(log)log.textContent=`ベノムフィールド：敵に毒${g.turnPoisonGain}`;window.abyssImpact?.('#enemySprite','poison')}};
...
window.tacticalCardMoved=(g,kind)=>{if(kind==='discard'&&has(g,'潮捨ての貝殻')&&!g.relicDiscardUsed){g.relicDiscardUsed=true;window.drawAbyssCards?.(1);window.abyssImpact?.('#playerSprite','draw')}if(kind==='exhaust'&&!g.relicExhaustUsed){g.relicExhaustUsed=true;if(has(g,'灰珊瑚')){block(g,5);window.abyssImpact?.('#playerSprite','guard')}if(has(g,'喰らう海溝')){g.energy+=1;window.drawAbyssCards?.(1);window.abyssImpact?.('#playerSprite','draw')}}};
