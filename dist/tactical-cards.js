...
window.beginAbyssTactics=g=>{g.primedAttack=0;g.firstCardEchoReady=!!g.doubleFirstCard&&(g.turn||1)>=(g.doubleFirstStartsTurn||1);g.tacticalDiscardUsed=false;g.tacticalExhaustUsed=false;g.energy+=(g.futureEnergy||0)+(has(g,'呪海の炉')?1:0)+(has(g,'六眼の王冠')?1:0);g.futureEnergy=0;if(g.turn===1&&has(g,'呪紋の貝殻'))block(g,Math.min(6,window.countAbyssCurses(g)*2))};
...
window.abyssTacticalStatus=g=>[[g.primedAttack,'次の攻撃＋'],[g.futureEnergy,'次ターン⚡＋'],[g.nextTurnBlock,'次ターン🛡'],[g.bankBlock,'持越し上限🛡'],[g.doubleFirstCard,'初手を二重発動'],[g.extraTurnDraw,'毎ターン＋1枚'],[g.curseInversion,'呪いを回復へ反転']].filter(([n])=>n).map(([n,label])=>'<span class="state-pill">'+label+(typeof n==='number'&&n!==1?n:'')+'</span>').join('');
window.abyssHandLimit=g=>has(g,'六眼の王冠')?6:8;
window.tacticalCardMoved=(g,kind)=>{if(kind==='discard'&&has(g,'漂流者の糸')&&!g.tacticalDiscardUsed){g.tacticalDiscardUsed=true;window.drawAbyssCards?.(1)}if(kind==='exhaust'&&has(g,'供物の真珠')&&!g.tacticalExhaustUsed){g.tacticalExhaustUsed=true;block(g,4)}};
})();
