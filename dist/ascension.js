...
function render(){let n=document.getElementById('ascensionNumber'),name=document.getElementById('ascensionName'),rule=document.getElementById('ascensionRule'),prev=document.getElementById('ascensionPrev'),next=document.getElementById('ascensionNext'),progress=document.getElementById('memoryProgress');if(!n)return;let r=RANKS[meta.selected];n.textContent=meta.selected;name.textContent=r[0];rule.textContent=r[1];prev.disabled=meta.selected<=0;next.disabled=meta.selected>=meta.unlocked;progress.textContent=`深海の記憶 ${meta.clears.length}/${RANKS.length}`;window.applyFuri?.(document.getElementById('ascensionPanel'))}
let panel=document.createElement('div');panel.id='ascensionPanel';panel.className='ascension-panel';panel.innerHTML='<div class="ascension-select"><button id="ascensionPrev" aria-label="前の深海階級">‹</button><div><small>深海階級</small><strong>A<span id="ascensionNumber">0</span>　<span id="ascensionName"></span></strong></div><button id="ascensionNext" aria-label="次の深海階級">›</button></div><p id="ascensionRule"></p><button class="memory-open" id="memoryOpen"><span>◉ 討伐記録</span><b id="memoryProgress"></b></button>';
