...
function read(){for(const key of [KEY,BACKUP])try{let x=JSON.parse(localStorage.getItem(key)||'null');if(valid(x)){x.unlocked=Math.min(10,Math.max(0,x.unlocked));x.selected=Math.min(x.unlocked,Math.max(0,x.selected||0));return x}}catch(e){}return fresh()}
...
let panel=document.createElement('div');panel.id='ascensionPanel';panel.className='ascension-panel';panel.innerHTML='<div class="ascension-select"><button id="ascensionPrev" aria-label="前の深淵階級">‹</button><div><small>深淵階級</small><strong>A<span id="ascensionNumber">0</span>　<span id="ascensionName"></span></strong></div><button id="ascensionNext" aria-label="次の深淵階級">›</button></div><p id="ascensionRule"></p><button class="memory-open" id="memoryOpen"><span>◉ 討伐記録</span><b id="memoryProgress"></b></button>';
document.querySelector('.title-actions')?.before(panel);
let modal=document.createElement('div');modal.className='modal';modal.id='memoryModal';modal.innerHTML='<div class="panel memory-panel modal-shell"><header class="modal-shell-head"><div><h2>深淵の記憶</h2><p>第3層ボスを倒すたび、沈んだ記憶が一つずつ目を覚ます。</p></div></header><div class="modal-shell-body memory-grid" id="memoryGrid"></div><footer class="modal-shell-foot"><button class="btn" id="memoryClose">閉じる</button></footer></div>';document.body.appendChild(modal);
