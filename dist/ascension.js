...
function read(){for(const key of [KEY,BACKUP])try{let x=JSON.parse(window.abyssStorageGet(key)||'null');if(valid(x)){x.unlocked=Math.min(DEAD_SEA_RANK,Math.max(0,x.unlocked));x.selected=Math.min(x.unlocked,Math.max(0,x.selected||0));return x}}catch(e){}return fresh()}
function write(m){try{let raw=JSON.stringify(m);window.abyssStorageSet(KEY,raw);window.abyssStorageSet(BACKUP,raw)}catch(e){}}
let meta=read();
...
window.recordAbyssClear=a=>{a=Math.min(DEAD_SEA_RANK,Math.max(0,a||0));let first=!meta.clears.includes(a);if(first)meta.clears.push(a);if(first&&a===0)try{window.abyssStorageSet('abyssShardGuidePending','1')}catch(e){}let old=meta.unlocked;if(a===meta.unlocked&&a<DEAD_SEA_RANK)meta.unlocked=a+1;if(meta.unlocked>old)meta.selected=meta.unlocked;write(meta);render();return{first,cards:first?(window.getAbyssUnlockCards?.(a)||[]):[],newRank:meta.unlocked>old?meta.unlocked:null,deadSeaUnlocked:meta.unlocked>old&&meta.unlocked===DEAD_SEA_RANK,memory:RANKS[a][3]}};
