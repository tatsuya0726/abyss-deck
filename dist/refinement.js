...
function syncPortraitEventArt(){if(!matchMedia('(orientation:portrait) and (max-width:700px)').matches)return;for(const [iconId,titleId]of [['eventIcon','eventTitle'],['mapChoiceIcon','mapChoiceTitle'],['deckChoiceIcon','deckChoiceTitle']]){const icon=document.getElementById(iconId),title=document.getElementById(titleId)?.textContent?.trim(),art=eventArt[title];if(!icon||!art||icon.querySelector('.event-illustration'))continue;icon.innerHTML=`<img class="event-illustration" src="assets/events/${art}.webp?v=3" alt="" aria-hidden="true">`}}
const portraitEventObserver=new MutationObserver(syncPortraitEventArt);for(const id of ['eventIcon','eventTitle','mapChoiceIcon','mapChoiceTitle','deckChoiceIcon','deckChoiceTitle']){const el=document.getElementById(id);if(el)portraitEventObserver.observe(el,{subtree:true,childList:true,characterData:true})}addEventListener('orientationchange',()=>setTimeout(syncPortraitEventArt,80));
