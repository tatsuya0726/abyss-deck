...
window.ABYSS_EVENTS?.push(
 ['🦴','鯨骨の墓場','巨大な鯨の骨が海底に横たわり、その内側で古い力が脈打っている。',[['骨の中へ入る','HPを10失い、遺物「古代の甲殻」を得る',()=>{let g=game();g.hp=Math.max(1,g.hp-10);addRelic('古代の甲殻')}],['ゴールドだけ拾う','ゴールドを61得る',()=>game().pearl+=61],['静かに祈る','HPを10回復',()=>{let g=game();g.hp=Math.min(g.max,g.hp+10)}]]],
 ...
 ['👻','幽霊船の航路','霧の中から幽霊船が現れた。船長は三つの航路を指し示す。',[['安全な航路','遺物「防毒ジャケット」を得る',()=>addRelic('防毒ジャケット')],['宝の航路','HPを9失い、ゴールド77を得る',()=>{let g=game();g.hp=Math.max(1,g.hp-9);g.pearl+=77}],['禁じられた航路','HPを12失い、深淵カードを得る',()=>{let g=game();g.hp=Math.max(1,g.hp-12);addCard(abyss)}]]],
