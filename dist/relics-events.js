...
const BOSS_RELICS={
 ...
 '深海炉心':['🌋','毎ターンのエナジーが1増える。ただし、1ターンに使えるカードは5枚まで。','boss'],
 ...
 '深海の瞳':['🌌','ターン開始時、追加でカードを2枚引く。ただし、すべてのカードのコストは引くたびにランダムになる。','boss']
};
...
window.ABYSS_EVENTS?.push(
 ...
 ['🥚','リヴァイアサンの卵','鼓動する巨大な卵。殻の奥から、深海の力が呼びかけてくる。',[['力を受け取る','HPを12失い、レアカードを得る',()=>{let g=game();g.hp=Math.max(1,g.hp-12);addCard(rare)}],['殻を身につける','最大HPが5増え、HPを5回復',()=>{let g=game();g.max+=5;g.hp+=5}],['瞳を抉る','最大HPを4失い、遺物「深海の瞳」を得る',()=>{let g=game();g.max=Math.max(20,g.max-4);g.hp=Math.min(g.hp,g.max);addRelic('深海の瞳')}]]],
