...
const BOSS_RELICS={
 ...
 '次元圧縮':['📦','入手時、デッキ内のすべての「フィン・スラッシュ」と「鱗の守り」をランダムなカードに変える。','boss'],
...
function grantBossRelic(name){let g=game(),r=BOSS_RELICS[name];if(!g||!r||g.relic.some(x=>x[1]===name))return false;g.relic.push([r[0],name]);g.pendingBossRelic=false;if(name==='巨鯨の心臓'){g.max+=18;g.hp+=18}if(name==='次元圧縮')window.transformAbyssBasicCards?.();return true}
...
window.ABYSS_EVENTS?.push(
 ...
 ['🥚','リヴァイアサンの卵','鼓動する巨大な卵。殻の奥から、深淵の力が呼びかけてくる。',[['力を受け取る','HPを12失い、レアカードを得る',()=>{let g=game();g.hp=Math.max(1,g.hp-12);addCard(rare)}],['殻を身につける','最大HPが5増え、HPを5回復',()=>{let g=game();g.max+=5;g.hp+=5}],['瞳を抉る','最大HPを4失い、遺物「深淵の瞳」を得る',()=>{let g=game();g.max=Math.max(20,g.max-4);g.hp=Math.min(g.hp,g.max);addRelic('深淵の瞳')}]]],
 ...
 ['👻','幽霊船の航路','霧の中から幽霊船が現れた。船長は三つの航路を指し示す。',[['安全な航路','遺物「防毒ジャケット」を得る',()=>addRelic('防毒ジャケット')],['宝の航路','HPを9失い、ゴールド70を得る',()=>{let g=game();g.hp=Math.max(1,g.hp-9);g.pearl+=70}],['禁じられた航路','HPを12失い、深淵カードを得る',()=>{let g=game();g.hp=Math.max(1,g.hp-12);addCard(abyss)}]]],
 ...
 ['🏺','漂着した宝物入れ','波間に漂う古い箱には、名も知らぬ遺物が眠っている。',[['箱を開ける','ランダムな遺物を1つ得る',()=>randomRelicGrant()],['そっと売り払う','ゴールドを50得る',()=>game().pearl+=50],['触れずに立ち去る','何も起こらない',()=>{}]]],
 ['🧳','漂流者の遺品','海底に沈んだ鞄の中に、まだ使える道具が残っている。',[['遺品を受け取る','HPを6失い、ランダムな遺物を得る',()=>{let g=game();g.hp=Math.max(1,g.hp-6);randomRelicGrant()}],['そのまま埋葬する','HPを10回復',()=>{let g=game();g.hp=Math.min(g.max,g.hp+10)}],['中身だけ確認する','ゴールドを30得る',()=>game().pearl+=30]]]
);
})();
