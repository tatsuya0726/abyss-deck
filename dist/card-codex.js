...
const PHOTO_ART={
  ...
  trick:'assets/cards/card-trick-v2.webp',venomfield:'assets/cards/card-venom-field-v2.webp',invisiblehand:'assets/cards/card-invisible-hand.webp',
...
let modal=document.createElement('div');modal.className='modal';modal.id='cardCodexModal';modal.innerHTML=`<div class="panel card-codex-panel modal-shell"><header class="modal-shell-head card-codex-head"><div><small>ABYSS CARD ARCHIVE</small><h2>カード図鑑</h2><p>全${KEYS.length}枚。カードを押すと強化後の表示に切り替わり、もう一度押すと元に戻ります。</p><div class="codex-controls"><div class="codex-filters"><button class="on" data-filter="all">すべて</button><button data-filter="normal">コモン</button><button data-filter="uncommon">アンコモン</button><button data-filter="rare">レア</button><button data-filter="abyss">深淵</button><button data-filter="special">特殊</button></div><div class="codex-sortbar" aria-label="カードの並べ替え"><span>並べ替え</span><button class="on" data-sort="archive">図鑑順</button><button data-sort="cost">コスト別</button><button data-sort="class">分類別</button></div></div></div></header><div class="modal-shell-body card-codex-body"><div class="card-codex-grid unified-card-grid" id="cardCodexGrid"></div></div><footer class="modal-shell-foot"><button class="btn" id="cardCodexClose">閉じる</button></footer></div>`;document.body.appendChild(modal);
...
const CLASS_ORDER={attack:0,counter:1,block:2,poison:3,heal:4,flow:5,power:6,special:7},CLASS_LABEL={attack:'⚔️ 攻撃',counter:'⚔️🛡️ 攻防',block:'🛡️ 防御',poison:'☠️ 毒',heal:'💚 回復',flow:'🌊 手札・山札操作',power:'✨ 永続強化',special:'◆ 特殊'},KEY_ORDER=Object.fromEntries(KEYS.map((k,i)=>[k,i]));
function classOf(k){let key=keyOf(k),c=data(key);return c.special||key==='abysscurse'||key==='treasuremap'||key==='rustkey'?'special':typeOf(key)}
function costOf(k){let cost=Number(data(k).c);return Number.isFinite(cost)?Math.max(0,cost):99}
function groupLabel(k,sort){if(sort==='cost'){let cost=costOf(k);return cost>=90?'使用不可':`${cost}コスト`}return sort==='class'?CLASS_LABEL[classOf(k)]||CLASS_LABEL.special:''}
function sortedKeys(filter,sort){let keys=KEYS.filter(k=>filter==='all'||rarity(data(k))===filter);return keys.sort((a,b)=>sort==='cost'?costOf(a)-costOf(b)||RARITY_ORDER[rarity(data(a))]-RARITY_ORDER[rarity(data(b))]||KEY_ORDER[a]-KEY_ORDER[b]:sort==='class'?(CLASS_ORDER[classOf(a)]??99)-(CLASS_ORDER[classOf(b)]??99)||costOf(a)-costOf(b)||KEY_ORDER[a]-KEY_ORDER[b]:filter==='all'?RARITY_ORDER[rarity(data(a))]-RARITY_ORDER[rarity(data(b))]||KEY_ORDER[a]-KEY_ORDER[b]:KEY_ORDER[a]-KEY_ORDER[b])}
function render(filter='all'){let grid=$('#cardCodexGrid'),keys=sortedKeys(filter,currentSort),lastGroup=null;grid.innerHTML=keys.map(k=>{let group=groupLabel(k,currentSort),heading=group&&group!==lastGroup?`<h3 class="codex-group-heading">${group}</h3>`:'';lastGroup=group;return `${heading}<button type="button" class="codex-card-wrap" data-codex-card="${k}" aria-pressed="false">${cardFace(k)}</button>`}).join('');grid.querySelectorAll('.codex-card-wrap').forEach(card=>card.onclick=()=>{let upgraded=card.classList.toggle('show-upgrade');card.setAttribute('aria-pressed',String(upgraded));card.innerHTML=cardFace(card.dataset.codexCard,upgraded);window.applyFuri?.(card)});window.applyFuri?.(modal)}
const FILTER_ORDER=['all','normal','uncommon','rare','abyss','special'];
let currentFilter='all',currentSort='archive';
...
function setSort(sort){currentSort=sort;modal.querySelectorAll('[data-sort]').forEach(x=>{let on=x.dataset.sort===sort;x.classList.toggle('on',on);x.setAttribute('aria-pressed',String(on))});render(currentFilter);modal.querySelector('.card-codex-body').scrollTop=0}
button.onclick=()=>{currentSort='archive';modal.querySelectorAll('[data-sort]').forEach(x=>{let on=x.dataset.sort==='archive';x.classList.toggle('on',on);x.setAttribute('aria-pressed',String(on))});setFilter('all');modal.classList.add('on');window.applyFuri?.(modal)};
...
modal.querySelectorAll('[data-sort]').forEach(b=>b.onclick=()=>setSort(b.dataset.sort));
