...
const SYNERGIES=[
 {icon:'🜏',name:'呪いと供物',tag:'呪い・廃棄・回収',text:'「深海の誘い灯」や「虚空顎」で呪いを増やし、「影喰いウツボ」で攻防へ変換する。手札に残した呪いの傷は「反転術式」で回復へ変えられる。',cards:['深海の誘い灯','虚空顎','影喰いウツボ','反転術式','呪潮の循環']},
 ...
 {icon:'🦈',name:'連撃コンボ',tag:'攻撃をつなぐ',cards:['魚群連撃','鮫群ラッシュ','追従する牙','コバンザメの力'],text:'攻撃力アップは多段攻撃の1発ごとに加わる。「追従する牙」は攻撃カードの直後に使うと2回攻撃。'},
 {icon:'🐚',name:'甲殻コンボ',tag:'守るほど強い',cards:['育つ甲殻','珊瑚の構え','群泳の守り','大潮の城壁'],text:'「珊瑚の構え」で敵の行動後に残るブロックを持ち越す。「大潮の城壁」は次ターンの防御を予約し、鱗盾チャージへつなげられる。'},
 {icon:'🌊',name:'手札循環コンボ',tag:'たくさん引く',cards:['逆巻く海流','飛魚の一閃','擬態の極意','群泳の守り'],text:'「擬態の極意」で山札の上3枚から1枚を選び、残りを捨て札へ。回収や鮫群ラッシュの強化にもつながる。「群泳の守り」は手札が多いほどブロックが大きくなる。'},
...
function renderGuide(){let m=window.getAbyssGame?.()?.runModifier,unlocked=new Set(meta.unlocked);$('#strategyContent').innerHTML='<div id="strategyArchive">'+modifierMarkup(m)+'<h3 class="archive-heading">⚓ シナジー図鑑</h3><div class="synergy-grid">'+SYNERGIES.map(s=>`<article class="synergy-card"><i>${s.icon}</i><div><b>${s.name}</b><small>${s.tag}</small><p>${s.text}</p><div class="combo-cards">${s.cards.map(c=>`<span>${c}</span>`).join('')}</div></div></article>`).join('')+'</div></div><div id="achievementArchive"><h3 class="archive-heading">🏆 称号・実績 <small>'+unlocked.size+'/'+ACH.length+'</small></h3><div class="achievement-grid">'+ACH.map(a=>`<article class="achievement-card ${unlocked.has(a.id)?'unlocked':'locked'} ${meta.selected===a.id?'selected':''}"><i>${unlocked.has(a.id)?a.icon:'？'}</i><div><b>${unlocked.has(a.id)?a.title:'未解放の称号'}</b><p>${a.desc}</p><small>${meta.selected===a.id?'最新の称号':unlocked.has(a.id)?'獲得済み':'条件を達成すると解放'}</small></div></article>`).join('')+'</div></div>';window.applyFuri?.(guide)}
function openGuide(section='strategy'){renderGuide();guide.classList.add('on');requestAnimationFrame(()=>{let body=$('#strategyContent'),target=$(section==='achievements'?'#achievementArchive':'#strategyArchive');if(body&&target)body.scrollTop=Math.max(0,target.offsetTop-body.offsetTop-8)})}window.openAbyssStrategy=openGuide;$('#titleCodex').onclick=()=>openGuide('strategy');$('#strategyClose').onclick=()=>guide.classList.remove('on');guide.onclick=e=>{if(e.target===guide)guide.classList.remove('on')};
