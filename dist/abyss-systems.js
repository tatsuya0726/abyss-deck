...
const SYNERGIES=[
 ...
 {icon:'◆',name:'深海刻印コンボ',tag:'エナジーを戻す',cards:['深海刻印','擬態の極意','追従する牙','深海の舞'],text:'刻印したカードは使うとエナジーが1戻る。ドローや連携カードへ刻むと、長いコンボを作りやすい。'}
];
const ACH=[
 ...
 {id:'collector',icon:'📚',title:'深海の収集家',desc:'デッキを30枚以上にする。',test:s=>s.maxDeck>=30},
...
function showModifier(m=window.getAbyssGame?.()?.runModifier){if(!m){setTimeout(()=>window.playPendingAbyssLayerIntro?.(),120);return}$('#modifierIcon').textContent=m.icon;$('#modifierName').textContent=m.name;$('#modifierBenefit').textContent=m.benefit;$('#modifierCost').textContent=m.cost;reveal.classList.add('on');window.applyFuri?.(reveal)}
...
let guide=document.createElement('div');guide.className='modal';guide.id='strategyModal';guide.innerHTML='<div class="panel strategy-panel modal-shell"><header class="modal-shell-head"><div><small>ABYSS ARCHIVE</small><h2>戦術図鑑・称号</h2></div><nav class="archive-tabs" id="archiveTabs"><button type="button" data-tab="strategy" class="active">⚓ 戦術</button><button type="button" data-tab="achievements">🏆 実績</button><button type="button" data-tab="memory">◉ 深海の記憶</button></nav></header><div class="modal-shell-body" id="strategyContent"></div><footer class="modal-shell-foot"><button class="btn" id="strategyClose">閉じる</button></footer></div>';document.body.appendChild(guide);
...
function modifierListMarkup(){let on=window.isAbyssRunModifierEnabled?.();return `<h3 class="archive-heading">🌊 深海異変一覧 <small>設定：${on?'ON':'OFF'}</small></h3><p class="archive-note">${on?'新しい潜航のたび、この中から1つがランダムに選ばれます。':'現在はOFFです。設定から深海異変をONにすると、新しい潜航に反映されます。'}</p><div class="synergy-grid">`+MODIFIERS.map(mm=>`<article class="synergy-card"><i>${mm.icon}</i><div><b>${mm.name}</b><p class="good">${mm.benefit}</p><p class="bad">${mm.cost}</p></div></article>`).join('')+'</div>'}
function renderGuide(){let m=window.getAbyssGame?.()?.runModifier,unlocked=new Set(meta.unlocked);$('#strategyContent').innerHTML='<div id="strategyArchive">'+modifierMarkup(m)+modifierListMarkup()+'<h3 class="archive-heading">⚓ シナジー図鑑</h3><div class="synergy-grid">'+SYNERGIES.map(s=>`<article class="synergy-card"><i>${s.icon}</i><div><b>${s.name}</b><small>${s.tag}</small><p>${s.text}</p><div class="combo-cards">${s.cards.map(c=>`<span>${c}</span>`).join('')}</div></div></article>`).join('')+'</div></div><div id="achievementArchive" hidden><h3 class="archive-heading">🏆 称号・実績 <small>'+unlocked.size+'/'+ACH.length+'</small></h3><div class="achievement-grid">'+ACH.map(a=>`<article class="achievement-card ${unlocked.has(a.id)?'unlocked':'locked'} ${meta.selected===a.id?'selected':''}"><i>${unlocked.has(a.id)?a.icon:'？'}</i><div><b>${unlocked.has(a.id)?a.title:'未解放の称号'}</b><p>${a.desc}</p><small>${meta.selected===a.id?'最新の称号':unlocked.has(a.id)?'獲得済み':'条件を達成すると解放'}</small></div></article>`).join('')+'</div></div><div id="memoryArchive" hidden><div class="memory-grid">'+(window.getAbyssMemoryMarkup?.()||'')+'</div></div>';window.applyFuri?.(guide)}
