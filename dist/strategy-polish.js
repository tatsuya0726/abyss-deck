...
window.showAbyssLayerIntro=act=>{const g=game();if(window.abyssGateOpening||g?.pendingAbyssGate||g?.act===4&&act!==4)return false;const names={1:['第1層','薄明の沈降海'],2:['第2層','忘れられた深海'],3:['第3層','奈落の王域'],4:['深淵領域','四つの欠片が開く海']},d=document.createElement('div'),n=names[act]||names[1];d.className='layer-intro';d.innerHTML=`<span>― ${n[0]} ―</span><b>${n[1]}</b>`;document.body.appendChild(d);requestAnimationFrame(()=>d.classList.add('on'));setTimeout(()=>d.classList.add('out'),1750);setTimeout(()=>d.remove(),2600);return true};
window.requestAbyssLayerIntro=act=>{const g=game();if(!g||act===4)return;g.pendingLayerIntro=act;window.abyssSave?.()};
window.playPendingAbyssLayerIntro=()=>{const g=game(),act=g?.pendingLayerIntro;if(!act)return false;delete g.pendingLayerIntro;window.abyssSave?.();if(window.abyssGateOpening||g?.pendingAbyssGate||g?.act===4)return false;return window.showAbyssLayerIntro?.(act)!==false};
