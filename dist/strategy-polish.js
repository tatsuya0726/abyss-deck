...
const styleLink=document.createElement('link');styleLink.rel='stylesheet';styleLink.href='abyss-endgame.css?v=119';document.head.appendChild(styleLink);
...
const SHARD_NODE_FLAGS={red:'redShardElite',blue:'blueShardEvent',purple:'purpleShardAnomaly'};
window.acquireAbyssShard=color=>{const g=game();if(!g||shards(g)[color])return false;shards(g)[color]=true;const flag=SHARD_NODE_FLAGS[color];if(flag&&Array.isArray(g.map))g.map.forEach(n=>n[flag]=false);window.abyssSave?.();window.syncAbyssShardHud?.();shardFlash(color);return true};
window.shouldEnterAbyssMap=g=>(g?.ascension||0)>=1&&Object.keys(SHARDS).every(k=>shards(g)[k]);
function normalizeMapSpecials(g){if(!Array.isArray(g?.map)||g.act>3)return;const area=g.map.filter(n=>n.row>=1&&n.row<=7&&n.type!=='treasure'),spread=()=>Math.random()<.5?2:3,minRow=type=>type==='shop'?3:type==='elite'?2:0,targets=[['elite',(g.ascension||0)>=5?(Math.random()<.5?6:7):(Math.random()<.5?4:5)],['event',Math.random()<.5?5:6],['anomaly',spread()],['shop',2],['rest',spread()]];for(const[type,count]of targets){let found=area.filter(n=>n.type===type).sort(()=>Math.random()-.5),kept=[],usedRows=new Set();for(const n of found){if(kept.length>=count)break;if(usedRows.has(n.row-1)||usedRows.has(n.row)||usedRows.has(n.row+1))continue;kept.push(n);usedRows.add(n.row)}found.forEach(n=>{if(!kept.includes(n))n.type='battle'});found=kept;while(found.length<count){let choices=area.filter(n=>n.type==='battle'&&n.row>=minRow(type)&&!usedRows.has(n.row-1)&&!usedRows.has(n.row)&&!usedRows.has(n.row+1));if(!choices.length)choices=area.filter(n=>n.type==='battle'&&n.row>=minRow(type)&&usedRows.has(n.row));if(!choices.length)choices=area.filter(n=>n.type==='battle'&&n.row>=minRow(type));if(!choices.length)choices=area.filter(n=>!targets.some(([special])=>n.type===special)&&n.type!=='boss'&&n.row>=minRow(type)&&!usedRows.has(n.row));let n=choices[Math.random()*choices.length|0];if(!n)break;n.type=type;found.push(n);usedRows.add(n.row)}}g.mapRulesVersion=1}
window.normalizeAbyssMap=normalizeMapSpecials;
window.decorateAbyssMap=g=>{if(g.act>3)return;normalizeMapSpecials(g);if((g.ascension||0)<1)return;if(!shards(g).red){let candidates=g.map.filter(n=>n.row>=5&&n.row<=7&&(n.type==='elite'||n.type==='battle')),chosen=candidates.filter(n=>n.type==='elite').sort(()=>Math.random()-.5)[0]||candidates.sort(()=>Math.random()-.5)[0];if(chosen){chosen.type='elite';chosen.redShardElite=true}}if(!shards(g).blue){let candidates=g.map.filter(n=>n.row>0&&n.row<8&&(n.type==='event'||n.type==='battle')),chosen=candidates.filter(n=>n.type==='event').sort(()=>Math.random()-.5)[0]||candidates.sort(()=>Math.random()-.5)[0];if(chosen){chosen.type='event';chosen.blueShardEvent=true}}if(!shards(g).purple){let candidates=g.map.filter(n=>n.row>0&&n.row<8&&(n.type==='anomaly'||n.type==='battle')),chosen=candidates.filter(n=>n.type==='anomaly').sort(()=>Math.random()-.5)[0]||candidates.sort(()=>Math.random()-.5)[0];if(chosen){chosen.type='anomaly';chosen.purpleShardAnomaly=true}}};
window.abyssNodeClass=n=>{const g=game();return n.redShardElite&&!shards(g).red?'red-shard-elite':n.blueShardEvent&&!shards(g).blue?'blue-shard-event':n.purpleShardAnomaly&&!shards(g).purple?'purple-shard-anomaly':n.type==='secretBoss'?(n.guardian?'secret-boss-node guardian-node':'secret-boss-node'):n.abyssPath?'abyss-path-node':''};
...
window.openAbyssGate=onDone=>{window.abyssGateOpening=true;window.stopAbyssMusicNow?.();window.enterAbyssMap?.({intro:false});const d=document.createElement('div'),steps=[['四つの欠片が共鳴する','赤、青、黄、紫の光が、沈んだ王の背後へ集まっていく。'],['封印の扉が姿を現す','欠片は古い紋章へ変わり、海そのものに亀裂を刻んだ。'],['深淵への道が開いた','扉の向こうから、誰かの笑い声と黒い潮が流れ込む。']];let page=0,locked=false;d.className='abyss-gate-reveal';d.innerHTML='<div class="abyss-gate"><div class="shard-orbit"><i class="red"></i><i class="blue"></i><i class="yellow"></i><i class="purple"></i><span></span></div><small>ABYSS GATE</small><b></b><p></p><em>画面をタップして進む</em></div>';const draw=()=>{d.dataset.page=String(page+1);d.querySelector('b').textContent=steps[page][0];d.querySelector('p').textContent=steps[page][1]};const advance=e=>{e?.preventDefault?.();if(locked)return;if(page<steps.length-1){page++;draw();return}locked=true;window.playAbyssGateRumble?.();d.classList.add('open');setTimeout(()=>{if(onDone)onDone();else window.enterAbyssMap?.();window.abyssGateOpening=false;d.remove();window.playAbyssMapMusic?.()},1100)};draw();document.body.appendChild(d);requestAnimationFrame(()=>d.classList.add('on'));d.onclick=advance;d.tabIndex=0;d.setAttribute('role','button');d.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();advance(e)}};return true};
function abyssStory(steps,onDone,className=''){
 ...
 const finish=(fast=false)=>{if(locked)return;locked=true;d.classList.add(fast?'story-skipped':'tear');setTimeout(()=>{d.remove();onDone?.()},fast?180:720)};
