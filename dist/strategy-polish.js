...
const styleLink=document.createElement('link');styleLink.rel='stylesheet';styleLink.href='abyss-endgame.css?v=115';document.head.appendChild(styleLink);
...
function abyssStory(steps,onDone,className=''){
 ...
 const draw=()=>{const s=steps[page];d.classList.toggle('story-normal-keeper',!!s[3]);d.innerHTML=`<div class="keeper-shadow"></div><small>${s[0]}</small><b>${s[1]}</b><p>${s[2]}</p><span class="story-progress">${page+1} / ${steps.length}</span><em class="story-tap">画面をタップして進む</em>`;window.applyFuri?.(d)};
