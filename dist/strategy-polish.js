...
const chainAFollow=['','海図師の秘密航路','助けた海図師が約束どおり待っていた。あなたのデッキを見て、進む戦い方に合う装備を差し出す。',[['魚群航路','群泳の旗を得る。多段攻撃を強化。',()=>{addRelic('群泳の旗');game().echoPromise=null;game().echoCartographerDone=true}],['重圧航路','圧力真珠を得る。高コストカードで防御を得る。',()=>{addRelic('圧力真珠');game().echoPromise=null;game().echoCartographerDone=true}],['臨界航路','生命臨界を得る。HPが半分以下で真価を発揮する。',()=>{addRelic('生命臨界');game().echoPromise=null;game().echoCartographerDone=true}]]];chainAFollow.storyWhen=g=>g.act>=2&&g.echoPromise==='cartographer';chainAFollow.linkedFollowup=true;
