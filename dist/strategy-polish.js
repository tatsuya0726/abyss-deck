...
const chainAFollow=['','海図師の秘密航路','助けた海図師が約束どおり待っていた。あなたのデッキを見て、進む戦い方に合う装備を差し出す。',[['魚群航路','群泳の旗を得る。多段攻撃を強化。',()=>{addRelic('群泳の旗');game().echoPromise=null;game().echoCartographerDone=true}],['逆鱗航路','竜の逆鱗を得る。所持済みなら圧力真珠を得る。',()=>{if(!addRelic('竜の逆鱗'))addRelic('圧力真珠');game().echoPromise=null;game().echoCartographerDone=true}],['臨界航路','生命臨界を得る。HPが半分以下で真価を発揮する。',()=>{addRelic('生命臨界');game().echoPromise=null;game().echoCartographerDone=true}]]];chainAFollow.storyWhen=g=>g.act>=2&&g.echoPromise==='cartographer';chainAFollow.storyPriority=true;
...
window.showAbyssLayerIntro=act=>{const g=game();if(rewardScreenOpen()||window.abyssGateOpening||g?.pendingAbyssGate||g?.act===4&&act!==4)return false;const names={1:['第1層','薄明の沈降海'],2:['第2層','忘れられた深海'],3:['第3層','奈落の王域'],4:['深淵領域','四つの欠片が開く海']},d=document.createElement('div'),n=names[act]||names[1];d.className='layer-intro';d.innerHTML=`<span>― ${n[0]} ―</span><b>${n[1]}</b>`;document.body.appendChild(d);requestAnimationFrame(()=>d.classList.add('on'));setTimeout(()=>d.classList.add('out'),1750);setTimeout(()=>d.remove(),2600);return true};
